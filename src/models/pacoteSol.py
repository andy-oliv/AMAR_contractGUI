...
#CLIENT INFO
client = Client("John Doe", "Rua aleatória, 209, Augustina, São Paulo",
                "john.doe@email.com", "08914536729")

#EVENT INFO
event = Event("aniversário de 35 anos do John Doe", "Casa de Festas Arco Verde", "30/10/2025",
              "19:30", 0, "cartão de crédito em 6x")

#PACKAGE INFO
package = Package("Sol", 1330, "3 horas", 0)

#CONTRACT DETAILS
extra_hour = "200,00"
extra_hour_spelled = "duzentos reais"
contract_date = "Janeiro de 2025"
...
personalInfo = document.add_paragraph()
personalInfo.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
personalInfo.add_run(os.getenv("PHOTOGRAPHER_NAME")).bold = True
personalInfo.add_run(', fotógrafa, brasileira, portadora da carteira de identidade nº ')
personalInfo.add_run(os.getenv("PHOTOGRAPHER_ID")).bold = True
personalInfo.add_run(', expedida pelo IFP, inscrita no CNPJ sob o número ')
personalInfo.add_run(os.getenv("PHOTOGRAPHER_CNPJ")).bold = True
personalInfo.add_run(', residente e domiciliada à ')
personalInfo.add_run(os.getenv("PHOTOGRAPHER_ADDRESS")).bold = True
personalInfo.add_run(', doravante denominada ')
personalInfo.add_run('CONTRATADO').bold = True
personalInfo.add_run(', e ')
personalInfo.add_run(f'{client.name} ').bold = True
personalInfo.add_run('CPF: ')
personalInfo.add_run(f'{client.cpf}').bold = True
personalInfo.add_run(', residente em ')
personalInfo.add_run(f'{client.address}').bold = True
personalInfo.add_run(', doravante denominado(a) ')
personalInfo.add_run('CONTRATANTE').bold = True
personalInfo.add_run(', resolvem celebrar o presente contrato pelas cláusulas e condições a seguir:')
...
clause_object.add_run(f', serão entregues por email para {client.email} em até 30 dias após a data do evento. Após a disponibilização das fotos digitais por email será solicitada a seleção das 50 fotos que serão impressas. Assim que a seleção for concluída será diagramada a caixinha personalizada. Após a diagramação será enviado um link para o ')
clause_object.add_run('CONTRATANTE').bold = True
clause_object.add_run(' para que possa ser dada a autorização na confecção da caixinha. Serão ')
clause_object.add_run('permitidas até 2 modificações ').bold = True
clause_object.add_run('no produto diagramado. Após a autorização, as 50 fotos serão enviadas para impressão e a caixinha será confeccionada. O prazo para envio dos produtos pelo fornecedor é de até 15 dias úteis, podendo esta estimativa sofrer alterações com base nos prazos do fornecedor. Após o recebimento da caixinha e das fotos, o kit completo com a caixinha personalizada, pendrive com as fotos digitais e as 50 fotos 10x15cm estarão disponíveis para retirada. ')
clause_object.add_run('A entrega do kit completo fica à cargo do CONTRATANTE ').bold = True
clause_object.add_run('podendo o mesmo retirar o kit em nosso estúdio na Av. Francisco Brochado da Rocha, 207 em horário agendado após a confecção do mesmo, ou solicitando algum serviço de entrega como o Uber Flash para o envio do kit para a sua residência.')
...
clause_payment.add_run(f'R$ {package.price} ').bold = True
clause_payment.add_run('pela prestação dos serviços descritos na cláusula anterior mais ')
clause_payment.add_run(f'R$ {event.commuting_fee}').bold = True
clause_payment.add_run(' de deslocamento, totalizando ')
clause_payment.add_run(f'R$ {package.price + event.commuting_fee}').bold = True
clause_payment.add_run(' sendo este valor efetivado via ')
clause_payment.add_run(f'{event.payment_type}.').bold = True
