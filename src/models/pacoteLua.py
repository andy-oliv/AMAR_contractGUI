...
#PACKAGE INFO
package = Package("Lua", 1686, "3 horas", 0)
...
clause_object.add_run(f', serão entregues por email para {client.email} em até 30 dias após a data do evento. Após a disponibilização das fotos digitais por email será solicitada a seleção das 40 fotos que serão impressas em formato 15x21cm. Após a seleção, o mini álbum será diagramado e um link será enviado para o ')
clause_object.add_run('CONTRATANTE').bold = True
clause_object.add_run('para possíveis modificações no mini álbum ')
clause_object.add_run('(permitidas até 2 modificações)').bold = True
clause_object.add_run('. Após a autorização do ')
clause_object.add_run('CONTRATANTE').bold = True
clause_object.add_run(', o mini álbum será produzido e as fotos serão impressas. O tempo de produção do fornecedor pode levar até 15 dias úteis, podendo esta estimativa sofrer alterações com base nos prazos do nosso fornecedor. Após este prazo, as fotos devidamente impressas no formato 15x21cm, o pendrive com as fotos digitais e o mini álbum com capa personalizada estarão disponíveis para retirada. ')
